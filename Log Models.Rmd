...
full.log <- glm(Subscription ~ ., family = 'binomial', data = BankAF.train[,-21])
...
Bank.Train.x <- model.matrix(Subscription ~ ., BankAF.train[,-21])
...
Bank.Test.x <- model.matrix(Subscription ~ ., BankAF.test[,-21])
...
names(p)[2] <- 'Preds'
p %>% group_by(Subs, Preds) %>% ggplot(aes(x = Preds, fill = Subs)) + geom_boxplot() + facet_wrap(~Subs)
#write.csv(data.frame(BankAF.test$Subscription,fit.pred.lasso), file = "F:/R For Real/Stats-2-Project-2/LassoPred.csv")

#fit.pred.step <- predict(step.log, newdata = BankAF.test[,-21], type= "response")
```
...
p = data.frame(Subs = BankAF.test$Subscription, Preds = fit.pred.origin)
names(p)[2] <- 'Preds'
p %>% group_by(Subs, Preds) %>% ggplot(aes(x = Preds, fill = Subs)) + geom_boxplot() + facet_wrap(~Subs)
