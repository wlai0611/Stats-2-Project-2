...
names(p)[2] <- 'Preds'
p %>% group_by(Subs, Preds) %>% ggplot(aes(x = Preds, fill = Subs)) + geom_boxplot()
...
plot(roc.step, col = "blue1", lty = 1, add = TRUE, colorize = TRUE)
plot(roc.origin, col = "darkgreen", lty = 1, add = TRUE, colorize = TRUE)
abline(a=0, b= 1)
