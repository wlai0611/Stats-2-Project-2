...
BankAF %>% ggplot(aes(x = euribor3m, fill = Subscription)) + geom_boxplot() + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) + 
  facet_wrap(facets = BankAF$Subscription) + xlab("euribor") + coord_flip()
...
# making at least double the calls in may as all other months
