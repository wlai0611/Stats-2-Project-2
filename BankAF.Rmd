...
# Full Model
mylogit.1 <- glm(Subscription ~ ., data = BankAF.train[,-7], family = binomial)
summary(mylogit.1)


# Make predictions
probabilities <- mylogit.1 %>% predict(BankAF.test, type = "response")
predicted.classes <- ifelse(probabilities < 0.1, "no", "yes")
# Model accuracy
...
predicted.classes <- ifelse(probabilities > 0.14, "yes", "no")
# Model accuracy
mean(predicted.classes==BankAF.test$Subscription)
summary(probabilities)
# Create table for conjfusion Matrix
...
library(ROCR)
results.lasso<-prediction(step.model, as.matrix(BankAF.test$Subscription),label.ordering=c("yes","no"))
...
modeltest1 <- glm(Subscription ~ age, data = BankAF.train, family = binomial)
summary(modeltest1)
