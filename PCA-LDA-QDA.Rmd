...
# Creates a numeric data set for PCA - if you want duration add it to the select list
...
# if duration is in BankAF.train.y <- BankAF.train.x[,11]
# if duration is in BankAF.train.x <- BankAF.train.x[,1:10]

# if duration is out BankAF.train.y <- BankAF.train.x[,10]
# if duration is in BankAF.train.x <- BankAF.train.x[,1:9]

# same changes should be mirrored in test set
...
# If duration is in plot 1:10
# if duration is out plot 1:9
...
```{r}
# QDA training Model -  not running on PCs using actual variables
fit.qda <- qda(BankAF.train.y ~ ., data = BankAF.train.x)

pred.qda <- predict(fit.qda, newdata = BankAF.train.x)
...
```{r}
pred.qda <- predict(fit.qda, newdata = BankAF.test.x)
...
```{r}
heplots::covEllipses(BankAF.train.x[,1:10], BankAF.train.y, variables = c(1:10))
```
